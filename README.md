...
依据《非诉网核工作台 PRD v1.0》实现 Project → Task → Query → Capture（用户界面称“留痕”）。Milestone 4 增加正式 Chrome Side Panel：对当前网页一键生成完整 PDF，并通过用户 JWT、现有 RLS 和 M3 协调协议归档到 Private Storage。详细交付、构建及验收见 [MILESTONE_4.md](MILESTONE_4.md)。
...
项目编辑/删除、Task 删除未增加 UI：本阶段指定的项目操作是新建/进入，Task 操作是新增/编辑/状态管理。数据库已允许所有者 CRUD，并验证删除级联。
